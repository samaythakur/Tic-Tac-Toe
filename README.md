...
# Project Flow
Start the application and initialize the game board.
Create a 3×3 grid using Tkinter buttons.
Assign Player X as the first player.
Detect button click events from the user.
Update the selected cell with the current player's symbol.
Switch turns dynamically between Player X and Player O.
...
Reset the game
Repeat gameplay for a new match.
